...
public class AdmlOfStructuresByUAI extends AdmlResourcesProvider {
	...
	public void authorizeAdml(HttpServerRequest resourceRequest, Binding binding,
		...
		List<String> structures = resourceRequest.params().getAll("uai");
		...
		String query =
				"MATCH (s:Structure) " +
				"WHERE s.UAI IN {structures} " +
				"WITH COLLECT(s.id) as sIds " +
				"RETURN LENGTH(FILTER(sId IN sIds WHERE sId IN {scope})) = LENGTH(sIds) as exists ";
		JsonObject params = new JsonObject()
				.put("structures", new JsonArray(structures.toArray()))
				.put("scope", new JsonArray(adminLocal.getScope().toArray()));
